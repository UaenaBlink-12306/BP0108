...
internal static class BP0108DisplayLauncher
{
        ...
        try
        {
            ...
            while (true)
            {
                ...
                using (Process game = Process.Start(startInfo))
                {
                    ...
                    // A normal user close exits cleanly. An unexpected crash is restarted after a
                    // short cool-down so the Twitch scene does not require human intervention.
                    if (exitCode == 0)
                    {
                        return 0;
                    }
    ...
    private static void RotateRuntimeLog(string runtimeLogPath)
    {
        ...
        {
            // Runtime log rotation must never block the stream.
        }
    }
